...
const validHeaders = (headers, reporter) => {
  ...
  return Object.entries(headers).every(([path, headersList]) => Array.isArray(headersList) && headersList.every(header => {
...
const transformLink = (manifest, publicFolder, pathPrefix) => header => header.replace(_constants.LINK_REGEX, (__, prefix, file, suffix) => {
  ...
  if ((0, _fsExtra.existsSync)(publicFolder(file))) {
...
const validateUserOptions = (pluginOptions, reporter) => headers => {
  ...
  if (typeof pluginOptions.transformHeaders !== 'function') {
    throw new TypeError(`The "transformHeaders" option to gatsby-plugin-netlify must be a function ` + `that returns an array of header strings. ` + `Check your gatsby-config.js.`);
  }
...
const mapUserLinkHeaders = ({
  ...
  pathPrefix,
  publicFolder
}) => headers => Object.fromEntries(Object.entries(headers).map(([path, headerList]) => [path, headerList.map(transformLink(manifest, publicFolder, pathPrefix))]));
...
const applyCachingHeaders = (pluginData, {
  ...
  const isComponentChunkSet = Boolean((_pluginData$component = pluginData.components.entries()) === null || _pluginData$component === void 0 ? void 0 : (_pluginData$component2 = _pluginData$component.next()) === null || _pluginData$component2 === void 0 ? void 0 : (_pluginData$component3 = _pluginData$component2.value[1]) === null || _pluginData$component3 === void 0 ? void 0 : _pluginData$component3.componentChunkName);
